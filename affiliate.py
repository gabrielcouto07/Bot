# affiliate.py - VERSÃO COMPLETA + JPEG
...
async def _ensure_csrf_token_from_affiliates(page) -> str | None:
    ...
    try:
        ...
        print(f"      → Navegando para página de afiliados (URL atual: {current_url[:60]}...)...")
        
        await page.goto("https://www.mercadolivre.com.br/afiliados", wait_until="domcontentloaded", timeout=60000)
        ...
        print(f"      ⚠️ Não encontrei CSRF token na página de afiliados")
        return None
...
async def _resolve_product_url(page, sec_url: str) -> str | None:
    try:
        ...
        if _is_product_page(url_inicial):
            print(f"      ✓ Já está na página do produto!")
            return url_inicial
        
        if "/social/" in url_inicial.lower() or "/minutoreview" in url_inicial.lower():
            print(f"      → Detectado minutoreview, clicando 'Ir para produto'...")
            
            clicked = await _click_ir_para_produto(page)
            
            if not clicked:
                print(f"      ⚠️ Botão 'Ir para produto' não encontrado")
                return None
            
            print(f"      ✓ Clicou em 'Ir para produto'")
            
            try:
                await page.wait_for_load_state("domcontentloaded", timeout=30000)
                print(f"      ✓ Navegação detectada")
            except Exception as e:
            ...
            if "MLB" in url_final.upper():
                print(f"      ✓ URL de produto válida (contém MLB)")
                return url_final
            else:
                print(f"      ⚠️ URL não parece ser de produto")
                return None
        
        if "MLB" in url_inicial.upper():
            print(f"      ✓ URL atual contém MLB, usando: {url_inicial[:120]}")
            return url_inicial
        
        print(f"      ✗ Não consegui chegar na página do produto")
        return None
...
async def _create_sec_via_api(page, product_url: str, tag: str) -> str | None:
    """
    🔥 Captura CSRF do domínio correto (afiliados)
    """
    ...
    print(f"      → Chamando API de afiliados...")
    try:
...
async def generate_affiliate_link(page_m, any_url: str, tag: str, max_retries: int = 2) -> tuple[str | None, str | None]:
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            # PASSO 1: Resolve URL do produto
...
async def download_product_image(page, product_url: str, out_dir: str = "./tmp") -> str | None:
    """
    🔥 Baixa imagem FORÇANDO formato JPEG (não PNG)
    Isso evita que WhatsApp converta em sticker
    """
    try:
        print(f"      → Baixando imagem do produto ML...")
        ...
        # 🔥 FORÇA JPEG + QUALIDADE ALTA
        await img_element.screenshot(
            path=str(out_path),
            type="jpeg",      # Remove transparência PNG
