...
class MLRotationManager:
    ...
    async def _open_extra_context(self, profile: dict):
        ...
        if not os.path.exists(user_data_dir):
            logger.warning(f"[MLRotation] Diretório não existe: {user_data_dir}")
            logger.warning(f"[MLRotation] Usando fallback para {profile['name']}")
            return
        
        logger.info(f"[MLRotation] Abrindo {profile['name']}...")
        ...
        try:
            ...
            # FIX BUG #5: Reduzido timeout de 90s para 30s + tratamento de falha
            try:
                await self._ml_page.goto(
                    "https://www.mercadolivre.com.br/afiliados",
                    wait_until="domcontentloaded",
                    timeout=30000,          # ← de 90s para 30s
                )
                logger.info(f"[MLRotation] ✅ {profile['name']} pronto!")
            except Exception as nav_err:
                logger.warning(f"[MLRotation] ⚠️ Navegação para afiliados falhou ({profile['name']}): {nav_err}")
                logger.warning(f"[MLRotation] Perfil pode não estar logado no ML — usando mesmo assim")
                # Não levanta exceção: contexto aberto ainda é válido para tentar gerar link
    ...
    async def get_ml_page_and_tag(self, page_m_main):
        ...
        - Se perfil dedicado não existe, usa page_m_main como fallback
        ...
        if self._ml_page is None:
            logger.warning(f"[MLRotation] ⚠️ Contexto dedicado não foi criado! Usando fallback (main context)")
            return page_m_main, prof["affiliate_tag"]
